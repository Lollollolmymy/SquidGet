...
#  include <windows.h>
#  include <conio.h>
// older toolchain headers don't have this
...
/* ── Orange palette (album mode) ── */
#define C_BORDER_O  "\033[38;5;130m"  /* dark orange  — box borders      */
#define C_LOGO_O    "\033[38;5;214m"  /* bright orange — title           */
#define C_HDR_O     "\033[38;5;172m"  /* amber        — column headers   */
#define C_SEL_NUM_O "\033[38;5;214m"  /* bright orange — selected rownum */
#define C_KEYNAME_O "\033[38;5;214m"  /* bright orange — keybind key     */
#define C_PROMPT_O  "\033[38;5;214m"  /* bright orange — "/" prompt      */
#define C_QSEL_O    "\033[38;5;214m"  /* bright orange — action selected */
#define C_CURSOR_O  "\033[38;5;208m"  /* orange       —  indicator       */
#define C_MODE_O    "\033[38;5;172m"  /* amber        — mode badge text  */

/* TC(s, blue, orange) — pick color based on search_type */
#define TC(st, b, o) ((st) == SEARCH_ALBUMS ? (o) : (b))
...
static void fb_ensure(AppState *s){
    ...
    if(!s->fb){s->fb_rows=0;s->fb_cols=0;return;}  /* guard: calloc failed */
...
void tui_render(AppState *s) {
    ...
    /* ── row 0: top border ╭── squidget ──╮ ── */
    {
        ...
        if (rd < 0) rd = 0;
        rb_reset();
        rb_s(TC(s->search_type, C_BORDER, C_BORDER_O)); rb_s(BTL);
        rb_rep(BH, ld);
        rb_s(A_BOLD); rb_s(TC(s->search_type, C_LOGO, C_LOGO_O)); rb_s(lbl); rb_s(A_RST); rb_s(TC(s->search_type, C_BORDER, C_BORDER_O));
        rb_rep(BH, rd);
        rb_s(BTR A_RST);
        fb_put(s, 0, rb_done());
    }
    ...
    {
        rb_reset();
        rb_s(TC(s->search_type, C_BORDER, C_BORDER_O)); rb_s(BV A_RST " ");
        int used = 2;
        ...
        const char *badge =
            ...
            s->mode == MODE_ALBUM_ACTION ? " ALBUM " :
                                      " QUALITY ";
        int bw = vw(badge);
        rb_s(C_MODEBG); rb_s(TC(s->search_type, C_MODE, C_MODE_O)); rb_s(A_BOLD); rb_s(badge); rb_s(A_RST " ");
        used += bw + 1;

        /* "/" prompt */
        rb_s(TC(s->search_type, C_PROMPT, C_PROMPT_O)); rb_s(A_BOLD "/" A_RST " ");
        used += 2;
        ...
        if ((s->mode == MODE_SEARCH || s->mode == MODE_RESULTS) && used < W - 2) {
            rb_s(TC(s->search_type, C_PROMPT, C_PROMPT_O)); rb_s("│" A_RST); used++;
        }
    ...
    {
        rb_reset();
        rb_s(TC(s->search_type, C_BORDER, C_BORDER_O)); rb_s(BLT A_RST);
        /* header content depends on search type */
        char hdr[512];
        if (s->search_type == SEARCH_ALBUMS)
            snprintf(hdr, sizeof(hdr), " %*s  %-*s  %-*s  %s",
                     COL_NUM, "#", tw, "album", aw, "artist", "trk");
        else
            snprintf(hdr, sizeof(hdr), " %*s  %-*s  %-*s  %s",
                     COL_NUM, "#", tw, "title", aw, "artist", "dur");
        char hdr_t[512];
        trunc_to(hdr, hdr_t, sizeof(hdr_t), inner);
        rb_s(TC(s->search_type, C_HDR, C_HDR_O)); rb_s(A_DIM); rb_s(hdr_t); rb_pad(inner - vw(hdr_t)); rb_s(A_RST);
        rb_s(TC(s->search_type, C_BORDER, C_BORDER_O)); rb_s(BRT A_RST);
        fb_put(s, 2, rb_done());
    }

    /* ── rows 3..H-3: track/album list ── */
    for (int r = 0; r < nlist; r++) {
        int idx    = s->scroll + r;
        int in_rng = (s->search_type == SEARCH_ALBUMS)
                        ? (idx < s->album_count)
                        : (idx < s->track_count);
        ...
        if (s->mode == MODE_SETUP) {
            ...
            if (r == 0) {
                ...
                if (pi >= 0 && pi < s->setup_count) {
                    ...
                    if (sel && is_browse)   rb_s(C_SEL_BG C_SEL_FG A_BOLD);
                    else if (sel)           rb_s(C_QSEL A_BOLD);
                    else if (is_browse)     rb_s(C_KEYNAME);
                    else                    rb_s(C_TITLE);
            ...
            if (r == 0) {
                ...
                rb_s(sel ? TC(s->search_type, C_QSEL, C_QSEL_O) : C_QDIM); if (sel) rb_s(A_BOLD);
                rb_s(ql); rb_pad(inner - vw(ql));
                rb_s(A_RST);
            } else if (r == 6) {
        ...
        } else if (s->search_type == SEARCH_ALBUMS && in_rng) {
            /* ── album list row ── */
            ...
            rb_s(" ");
            rb_s(is_sel ? TC(s->search_type, C_SEL_NUM, C_SEL_NUM_O) : C_NUM); if (is_sel) rb_s(A_BOLD);
            rb_s(nstr);
            rb_s(A_RST);
            if (is_sel) rb_s(C_SEL_BG);
            rb_s(" ");
            ...
            /* indicator */
            if (is_sel) { rb_s(TC(s->search_type, C_CURSOR, C_CURSOR_O)); rb_s(A_BOLD IND " " A_RST C_SEL_BG); }
            else         { rb_s("  "); }
            ...
            rb_s(atitl); rb_pad(tw - vw(atitl) + 1);
            ...
            /* track count */
            char trkcnt[8];
            if (s->albums[idx].num_tracks > 0)
                snprintf(trkcnt, sizeof(trkcnt), "%3d", s->albums[idx].num_tracks % 1000);
            else
                snprintf(trkcnt, sizeof(trkcnt), "  ?");
            rb_s(is_sel ? C_SEL_FG : C_DUR);
            rb_s(trkcnt);

            int row_used = 1 + COL_NUM + 1 + COL_IND + tw + 1 + aw + 1 + vw(trkcnt);
            rb_pad(inner - row_used);
            rb_s(A_RST);

        } else if (s->search_type == SEARCH_SONGS && in_rng) {
            /* ── track list row ── */
            if (is_sel) rb_s(C_SEL_BG);
    ...
    {
        rb_reset();
        rb_s(TC(s->search_type, C_BORDER, C_BORDER_O)); rb_s(BBL);
        rb_rep(BH, inner);
        rb_s(BBR A_RST);
...
#define K(key,val) kb[nkb].k=(key);kb[nkb].v=(val);nkb++
        if (s->mode == MODE_SEARCH || s->mode == MODE_RESULTS) {
            K("↑↓","nav"); K("enter","select"); K("^U","clear");
            K("tab", s->search_type == SEARCH_ALBUMS ? "songs" : "albums");
            K("^C","quit");
        } else if (s->mode == MODE_QUALITY) {
            K("↑↓","move"); K("1-5","pick"); K("enter","dl"); K("esc","cancel");
        } else if (s->mode == MODE_ALBUM_ACTION) {
            K("↑↓","move"); K("1-2","pick"); K("enter","confirm"); K("esc","cancel");
        } else if (s->mode == MODE_SETUP) {
            K("↑↓","move"); K("1-4","shortcut"); K("enter","confirm");
        }
#undef K

        const char *kname_c = TC(s->search_type, C_KEYNAME, C_KEYNAME_O);
        for (int i = 0; i < nkb; i++) {
            rb_s(kname_c); rb_s(A_BOLD); rb_s(kb[i].k); rb_s(A_RST);
