...
int gui_pick_folder(char *buf, size_t bufsz) {
    /* PowerShell one-liner: opens the native Windows folder browser.
     * Works on Windows 7+ without any extra dependencies.
     * We use IFileDialog via PowerShell so we get the modern Vista-style
     * dialog with sidebar shortcuts, favourites, etc.                    */
    const char *ps =
...
int gui_pick_folder(char *buf, size_t bufsz) {
    /* osascript "choose folder" returns a HFS+ path like:
     *   Macintosh HD:Users:alice:Music:squidget:
     * The "POSIX path of" coercion converts it to a POSIX path.
     * We strip the trailing "/" that osascript appends.               */
    FILE *p = popen(
        "osascript -e "
...
/* Try running a shell command and capturing its first output line.
 * Returns 1 and fills buf if successful, 0 otherwise.                */
